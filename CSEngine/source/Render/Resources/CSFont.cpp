...
bool CSFont::LoadFromFile(const bchar*name) 
{
	BearFileStream file;
	if (file.Open(name))
	{
...
bool CSFont::LoadFromStream(const BearInputStream & stream)
{
...
bool CSFont::LoadFromBuffer(const BearBufferedReader & stream)
{
	return LoadFromStream(stream);
}

bool CSFont::SaveToFile(const bchar * name)
{
	BearFileStream file;
...
bool CSFont::Empty() const
{
	return m_image.Empty() || m_chars_info.size() == 0;
}

BearFactoryPointer<BearRHI::BearRHITexture2D> CSFont::CreateTexture() const
{
	if (m_image.Empty())
...
#ifdef DEVELOPER_VERSION
bool CSFont::LoadFromTTF(const bchar * file, const bchar16 * chars, bsize width, bsize height, bsize size)
{
