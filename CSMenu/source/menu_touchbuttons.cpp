...
void UI_TouchButtons_AddButtonToList( const char *name, const char *texture, const char *command, unsigned char *color, int flags )
{
	...
	strcpy_s(uiTouchButtons.bNames[i], name);
	uiTouchButtons.bNamesPtr[i] = uiTouchButtons.bNames[i];
	strcpy_s(uiTouchButtons.bTextures[i], texture);
	strcpy_s(uiTouchButtons.bCommands[i], command);
...
{
	strcpy_s( uiTouchButtons.selectedName, uiTouchButtons.bNames[uiTouchButtons.buttonList.curItem]);
	strcpy_s( uiTouchButtons.texture.buffer, uiTouchButtons.bTextures[uiTouchButtons.buttonList.curItem]);
	strcpy_s( uiTouchButtons.command.buffer, uiTouchButtons.bCommands[uiTouchButtons.buttonList.curItem]);
	...
	uiTouchButtons.texture.cursor = static_cast<int>(strlen( uiTouchButtons.texture.buffer ));
	...
	uiTouchButtons.command.cursor = static_cast<int>(strlen( uiTouchButtons.command.buffer ));
...
static void UI_TouchButtons_FileDialogCallback( bool success )
{
	if( success )
	{
		strcpy_s( uiTouchButtons.texture.buffer, uiFileDialogGlobal.result );
		uiTouchButtons.textureid = PIC_Load(uiTouchButtons.texture.buffer);
	}
...
static void UI_TouchButtons_Callback( void *self, int event )
{
	...
	case ID_RESET:
        ...
        strcpy_s( uiTouchButtons.dialogText, "Reset all buttons?" );
	...
	case ID_DELETE:
        ...
        uiTouchButtons.no.generic.flags &= ~QMF_HIDDEN;
        uiTouchButtons.yes.generic.flags &= ~QMF_HIDDEN;
        strcpy_s( uiTouchButtons.dialogText, "Delete selected button?" );
	...
	case ID_SELECT:
		...
		strcpy_s( uiFileDialogGlobal.patterns[0], "touch/*.tga");
		strcpy_s( uiFileDialogGlobal.patterns[1], "touch_default/*.tga");
		strcpy_s( uiFileDialogGlobal.patterns[2], "gfx/touch/*");
		strcpy_s( uiFileDialogGlobal.patterns[3], "gfx/vgui/*");
		strcpy_s( uiFileDialogGlobal.patterns[4], "gfx/shell/*");
		strcpy_s( uiFileDialogGlobal.patterns[5], "*.tga");
