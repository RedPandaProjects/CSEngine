...
static void UI_PlayerSetup_FindModels( void )
{
	...
	strcpy_s( uiPlayerSetup.models[uiPlayerSetup.num_models], "player" );
	...
	for( i = 0; i < numFiles; i++ )
	{
		...
		sprintf_s( path, "models/player/%s/%s.mdl", name, name );
		if( !FILE_EXISTS( path )) continue;

		strcpy_s( uiPlayerSetup.models[uiPlayerSetup.num_models], name );
		uiPlayerSetup.num_models++;
	}
}
...
static void UI_PlayerSetup_GetConfig( void )
{
	...
	strncpy_s( uiPlayerSetup.name.buffer, CVAR_GET_STRING( "name" ), sizeof( uiPlayerSetup.name.buffer ));
	...
	strcpy_s( uiPlayerSetup.currentModel, uiPlayerSetup.models[(int)uiPlayerSetup.model.curValue] );
...
static void UI_PlayerSetup_UpdateConfig( void )
{
	...
	strcpy_s( name, uiPlayerSetup.models[(int)uiPlayerSetup.model.curValue] );

	if( !stricmp( name, "player" ))
	{
		strcpy_s( path, "models/player.mdl" );
		newImage[0] = '\0';
	}
	else
	{
		sprintf_s( path, "models/player/%s/%s.mdl", name, name );
		sprintf_s( newImage, "models/player/%s/%s.bmp", name, name );
	}
	...
	if( !ui_showmodels->value )
	{
		if( stricmp( lastImage, newImage ))
		{
			...
			if( stricmp( name, "player" ))
			{
				sprintf_s( lastImage, "models/player/%s/%s.bmp", name, name );
