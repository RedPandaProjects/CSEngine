...
static void UI_Controls_ParseKeysList( void )
{
	...
	while(( pfile = COM_ParseFile( pfile, token )) != NULL )
	{
		...
		if( !stricmp( token, "blank" ))
		{
			...
			sprintf_s( str, "^6%s^7", token );	// enable uiPromptTextColor
			...
			strcpy_s( uiControls.keysBind[i], "" );
			strcpy_s( uiControls.firstKey[i], "" );
			strcpy_s( uiControls.secondKey[i], "" );
			i++;
		}
		else
		{
			...
			strncpy_s( uiControls.keysBind[i], token, sizeof( uiControls.keysBind[i] ));
			...
			if( keys[0] == -1 ) strcpy_s( uiControls.firstKey[i], "" );
			else strncpy_s( uiControls.firstKey[i], KEY_KeynumToString( keys[0] ), sizeof( uiControls.firstKey[i] ));

			if( keys[1] == -1 ) strcpy_s( uiControls.secondKey[i], "" ); 
			else strncpy_s( uiControls.secondKey[i], KEY_KeynumToString( keys[1] ), sizeof( uiControls.secondKey[i] ));
			...
			if( !strnicmp( uiControls.firstKey[i], "MOUSE", 5 ))
				sprintf_s( str, "^5%s^7", uiControls.firstKey[i] );	// cyan
			else sprintf_s( str, "^3%s^7", uiControls.firstKey[i] );	// yellow
			...
			if( !strnicmp( uiControls.secondKey[i], "MOUSE", 5 ))
				sprintf_s( str, "^5%s^7", uiControls.secondKey[i] );// cyan
			else sprintf_s( str, "^3%s^7", uiControls.secondKey[i] );	// yellow
