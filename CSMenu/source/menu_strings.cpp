...
void UI_InitAliasStrings( void )
{
	...
	sprintf_s( token, "Quit %s without\nsaving current game?", gMenu.m_gameinfo.title );
	MenuStrings[HINT_QUIT_ACTIVE] = StringCopy( token );

	sprintf_s( token, "Learn how to play %s", gMenu.m_gameinfo.title );
	MenuStrings[HINT_HAZARD_COURSE] = StringCopy( token );

	sprintf_s( token, "Play %s on the 'easy' skill setting", gMenu.m_gameinfo.title );
	MenuStrings[HINT_SKILL_EASY] = StringCopy( token );

	sprintf_s( token, "Play %s on the 'medium' skill setting", gMenu.m_gameinfo.title );
	MenuStrings[HINT_SKILL_NORMAL] = StringCopy( token );

	sprintf_s( token, "Play %s on the 'difficult' skill setting", gMenu.m_gameinfo.title );
	MenuStrings[HINT_SKILL_HARD] = StringCopy( token );

	sprintf_s( token, "Quit playing %s", gMenu.m_gameinfo.title );
	MenuStrings[HINT_QUIT_BUTTON] = StringCopy( token );

	sprintf_s( token, "Search for %s servers, configure character", gMenu.m_gameinfo.title );
