...
void UI_LoadBackgroundImage( void )
{
	...
	for( int y = 0; y < BACKGROUND_ROWS; y++ )
	{
		for( int x = 0; x < BACKGROUND_COLUMNS; x++ )
		{
			sprintf_s( filename, "resource/background/800_%d_%c_loading.tga", y + 1, 'a' + x );
...
bool UI_StartBackGroundMap( void )
{
	...
	char cmd[128];
	sprintf_s( cmd, "maps/%s.bsp", uiStatic.bgmaps[bgmapid] );
...
void UI_AddServerToList( netadr_t adr, const char *info )
{
	...
	strncpy_s( uiStatic.serverNames[uiStatic.numServers], info, sizeof( uiStatic.serverNames[uiStatic.numServers] ));
	uiStatic.numServers++;
}
...
static void UI_LoadBackgroundMapList( void )
{
	...
	while(( pfile = COM_ParseFile( pfile, token )) != NULL )
	{
		...
		strncpy_s( uiStatic.bgmaps[uiStatic.bgmapcount], token, sizeof( uiStatic.bgmaps[0] ));
