...
void PM_SwapTextures(int i, int j)
{
	...
	strcpy_s(szTemp,17, grgszTextureName[i]);
	chTemp = grgchTextureType[i];

	strcpy_s(grgszTextureName[i], 17, grgszTextureName[j]);
	grgchTextureType[i] = grgchTextureType[j];

	strcpy_s(grgszTextureName[j], 17, szTemp);
...
void PM_InitTextureTypes(void)
{
	...
	while (pmove->memfgets(pMemFile, fileSize, &filePos, buffer, 511) != NULL && (gcTextures < CTEXTURESMAX))
	{
		...
		buffer[j] = 0;
		strcpy_s(&(grgszTextureName[gcTextures++][0]), 17, &(buffer[i]));
	}
...
void PM_CatagorizeTextureType(void)
{
	...
	strcpy_s(pmove->sztexturename, 256, pTextureName);
