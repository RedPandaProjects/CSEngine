...
void DBG_AssertFunction( BOOL fExpr, const char* szExpr, const char* szFile, int szLine, const char* szMessage )
{
	...
	else sprintf_s( szOut, "ASSERT FAILED:\n %s \n(%s@%d)", szExpr, szFile, szLine );
	HOST_ERROR( szOut );
}
#endif	// DEBUG
...
void COM_FileBase ( const char *in, char *out )
{
	...
	// Copy partial string
	strncpy_s( out, len+1, &in[start], len );
	// Terminate it
	out[len] = 0;
}
...
void UI_CheckBox_Draw( menuCheckBox_s *cb )
{
	...
	UI_DrawString( textOffset, y, static_cast<int>(strlen( cb->generic.name )) * cb->generic.charWidth, cb->generic.charHeight, cb->generic.name, uiColorHelp, true, cb->generic.charWidth, cb->generic.charHeight, justify, shadow );
	...
	if( cb->generic.statusText && cb->generic.flags & QMF_NOTIFY )
	{
		...
		x = 250;
		w = UI_SMALL_CHAR_WIDTH * static_cast<int>(strlen( cb->generic.statusText ));
...
void UI_Field_Init( menuField_s *f )
{
	...
	f->cursor = static_cast<int>(strlen( f->buffer ));
}
...
const char *UI_Field_Key( menuField_s *f, int key, int down )
{
	...
	len = static_cast<int>(strlen( f->buffer) );
...
void UI_Field_Draw( menuField_s *f )
{
	...
	len = static_cast<int>(strlen( f->buffer ) )+ 1;
	...
	if( a->generic.width < 1 || a->generic.height < 1 )
	{
		...
		else
		{
			if( a->generic.width < 1 )
				a->generic.width = a->generic.charWidth * static_cast<int>(strlen( a->generic.name) );
...
void UI_Action_Draw( menuAction_s *a )
{
	...
	if( a->generic.statusText && a->generic.flags & QMF_NOTIFY )
	{
		...
		x = 290;
		w = UI_SMALL_CHAR_WIDTH * static_cast<int>(strlen( a->generic.statusText) );
		UI_ScaleCoords( &x, NULL, &w, NULL );
		x += a->generic.x;
...
void UI_PicButton_Init( menuPicButton_s *pb )
{
	...
	if( pb->generic.width < 1 || pb->generic.height < 1 )
	{
		if( pb->generic.width < 1 )
			pb->generic.width = pb->generic.charWidth * static_cast<int>(strlen( pb->generic.name ));
...
void UI_PicButton_Draw( menuPicButton_s *item )
{
	...
	if( item->generic.statusText && item->generic.flags & QMF_NOTIFY )
	{
		...
		x = 290;
		w = UI_SMALL_CHAR_WIDTH * static_cast<int>(strlen( item->generic.statusText ));
		UI_ScaleCoords( &x, NULL, &w, NULL );
		x += item->generic.x;
