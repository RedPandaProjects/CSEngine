...
static void UI_CreateGame_Begin( void )
{
	...
	if( uiCreateGame.dedicatedServer.enabled )
	{
		...
		char cmd[128];
		sprintf_s( cmd, "#%s", gMenu.m_gameinfo.gamefolder );
...
static void UI_CreateGame_Init( void )
{
	...
	strcpy_s( uiCreateGame.hostName.buffer, CVAR_GET_STRING( "hostname" ));
	...
	if( CVAR_GET_FLOAT( "maxplayers" ) <= 1 )
		strcpy_s( uiCreateGame.maxClients.buffer, "8" );
	else sprintf_s( uiCreateGame.maxClients.buffer, "%i", (int)CVAR_GET_FLOAT( "maxplayers" ));
